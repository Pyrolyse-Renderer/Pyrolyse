...
Mesh load_obj(const char* path)
{
    ...
    for (std::size_t i = 0; i < ts; i++) triarray[i] = tris[i];
...
{
    out_mesh_count = 1;
    const Mesh m = load_obj("D:/Pyrolyse/samples/test.pyrobj");
    const auto meshes = new Mesh[out_mesh_count];
    meshes[0] = m;
    out_triangle_count = meshes[0].ntri;
    return meshes;
}
